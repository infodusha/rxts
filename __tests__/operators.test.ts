...
describe('Operators', () => {
  it('should work with tap', async () => {
    ...
    from([1, 2, 3]).pipe(tap(tapCall)).subscribe();
    await tick();
  ...
  it('should work with map', async () => {
    const next = jest.fn();
    from([1, 2, 3]).pipe(map((x) => x * 2)).subscribe({ next });
    await tick();
  ...
  it('should work with filter', async () => {
    const next = jest.fn();
    from([1, 2, 3, 4]).pipe(filter((x) => x > 2)).subscribe({ next });
    await tick();
  ...
  it('should work with delay', async () => {
    const next = jest.fn();
    from([1, 2, 3]).pipe(delay(100)).subscribe({ next });
    await tick();
